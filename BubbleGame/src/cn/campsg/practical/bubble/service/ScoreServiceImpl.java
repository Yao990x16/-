...
public class ScoreServiceImpl implements ScoreService {
	...
	public int getScoreByStars(int stars) {
		...
		int x = (int) (LOWER_SCORE * Math.pow(stars, 2));
	...
	public int getAwardScore(int leftStarNum) {
		...
		if(leftStarNum<AWARD_LIMIT) {
			return (int) (LOWER_AWARD_SCORE * Math.pow(AWARD_LIMIT - leftStarNum, 2));
		}
		else
		return 0;
	...
	public boolean isChangeLevel(int score) {
		...
		int x = c.getScore().getLevelScore();
		//判断当前游戏积分是否达到关卡通关目标分
		if(score >= x) {
			return true;
		}
	...
	public boolean isNoticePassLevel(int currentLevel, int score) {
		//如果当前积分未达到通关目标分,则不需要进行通关提示
		if(isChangeLevel(score)==false ) 
			return false;
		//判断游戏总通关次数是否等于当前关卡号
		//不相等:表示已经提示过通关信息,无需再次提示,直接返回false
		//相等:表示未提示过通关信息,则将游戏总通关次数+1,并返回true告知游戏界面需要提示通关消息
		if(zcs==currentLevel) {
			zcs++;
			return true;
		}
